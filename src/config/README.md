...
*   **Local LLM Runtime**: `DEFAULT_LLM_PROVIDER`, `DEFAULT_LLM_MODEL`, `LLAMA_CPP_BASE_URL`, `LLAMA_CPP_API_KEY`, `LLAMA_CPP_SERVER_BIN`, and `LLAMA_CPP_STARTUP_TIMEOUT` configure the llama.cpp-backed generation client. The default model is `unsloth/gemma-4-12B-it-qat-GGUF:UD-Q4_K_XL`, and the backend can start a managed `llama-server` for known registry models when the UI allows it. Per-model generation tuning such as context size, max tokens, sampling, JSON mode, and thinking is centralized in `src/pipeline/04_generation/llama_cpp_models.py`.
